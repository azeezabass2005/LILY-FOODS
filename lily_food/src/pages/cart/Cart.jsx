...
const Cart = () => {
  ...
  const incrementQuantity = (index) => {
    ...
    setProductInCart(updatedProductCart)
  }
  let updatedProductCart = []
  const decrementQuantity = (index) => {
    updatedProductCart = [...productInCart]
    if(updatedProductCart[index].quantity > 1) {
      ...
      setNumberInCart(updatedProductCart.length)
      setProductInCart(updatedProductCart)
    }
  ...
  if (productInCart.length > 0) return (
    <div className='sm:px-20 md:px-28 p-3 pt-36 sm:pt-36 lg:pt-44 md:pt-56 flex md:justify-between justify-center items-center flex-col min-h-[70vh]'>
      <div className={` ${showOrderForm ? 'blur-[5px]' : 'blur-0'} bg-[#dab9f9] gap-[1rem] flex flex-col vs:p-4 rounded-[20px] product-shadow`}>
        {productInCart.map((cartProduct, index) => (
          ...
          <div className='flex justify-center items-center vs:gap-[1rem] gap-[.3rem] font-semibold p-2'>
            <div className='h-[30px] w-[30px] bg-[#8a2be2] rounded-full text-white font-semibold text-[1.2rem] flex justify-center items-center cursor-pointer' onClick={() => incrementQuantity(index)}>+</div>
            <span className='text-[#8a2be2] text-[1.4rem]'>{cartProduct.quantity}</span>
            <div className='h-[30px] w-[30px] bg-[#8a2be2] rounded-full text-white font-semibold text-[1.2rem] flex justify-center items-center cursor-pointer' onClick={() => decrementQuantity(index)}>-</div>
          </div>
          </div>
        ))}
        <button className='p-2 vs:p-3 w-[90%] ml-[5%] bg-[#8a2be2] rounded-[10px]' onClick={() => setShowOrderForm((prev) => (!prev))}>place order</button>
      </div>
    ...
    <div className='sm:px-20 md:px-28 p-3 pt-36 sm:pt-36 lg:pt-44 md:pt-56 flex md:justify-between justify-center items-center flex-col min-h-[70vh]'>
      <div className='flex flex-wrap sm:justify-around justify-center items-center gap-[2rem] bg-[#dab9f9] vs:w-[85%] sm:w-[80%] w-[90%] mt-7 mb-4 p-5 sm:p-8 rounded-[10px]'>
